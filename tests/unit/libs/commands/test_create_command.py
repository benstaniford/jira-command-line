...
class TestCreateCommand:
    ...
    def test_execute_backlog_issue_creation(self, mock_ui, mock_view, mock_jira_api):
        ...
        mock_jira_api.get_backlog_issues.return_value = [mock_issue]
    ...
    def test_execute_sprint_issue_creation(self, mock_ui, mock_view, mock_jira_api):
        ...
        mock_jira_api.get_sprint_issues.return_value = [mock_issue]
        mock_jira_api.get_possible_types.return_value = ["Story", "Bug", "Task"]
    ...
    def test_execute_sub_task_creation(self, mock_ui, mock_view, mock_jira_api):
        ...
        mock_jira_api.get_child_issues.return_value = [mock_subtask]
    ...
    def test_execute_epic_child_creation(self, mock_ui, mock_view, mock_jira_api):
        """Test creating an issue inside an epic, which asks for a type first"""
        mock_view.mode = ViewMode.TASKVIEW
        mock_view.parent_issue = Mock()
        mock_view.parent_issue.key = "EPIC-1"
        mock_jira_api.is_epic.return_value = True

        mock_ui.prompt_get_string.side_effect = ["Story Summary", "Story Description"]
        mock_ui.prompt_with_choice_list.return_value = [0, "Story"]
        mock_jira_api.get_possible_types.return_value = ["Story", "Bug", "Task"]

        mock_child = Mock()
        mock_child.key = "TEST-321"
        mock_jira_api.create_sub_task.return_value = mock_child
        mock_jira_api.get_child_issues.return_value = [mock_child]

        command = CreateCommand()
        result = command.execute(ui=mock_ui, view=mock_view, jira=mock_jira_api)

        mock_jira_api.create_sub_task.assert_called_once_with(
            mock_view.parent_issue, "Story Summary", "Story Description", "Story"
        )
        mock_jira_api.create_backlog_issue.assert_not_called()
        mock_ui.prompt.assert_called_with("Created TEST-321...")
        assert result is False
    ...
    def test_execute_with_editor_f1(self, mock_editor, mock_ui, mock_view, mock_jira_api):
        ...
        mock_jira_api.create_backlog_issue.assert_called_once_with("Summary", "Editor Description", "Story", None, None)
        assert result is False
    ...
    def test_execute_with_f2_use_summary(self, mock_ui, mock_view, mock_jira_api):
        ...
        mock_view.rebuild.assert_called()
        mock_jira_api.create_backlog_issue.assert_called_once_with("Test Summary", "Test Summary", "Story", None, None)
        assert result is False
