#!/usr/bin/env python3
"""
Get failed tests from the runs visible in the Azure DevOps portal.
Uses the most recent run IDs based on ID number (highest = newest).
"""
...
def get_failures_by_run_ids(build_id, run_filters=None):
    """Get failed tests from specific runs."""
    ...
    for run_name in sorted(latest_runs.keys()):
        ...
        if failures:
            print(f"{run_name} (Run ID: {run_id}): {len(failures)} failure(s)")
            for test in failures:
                print(f"  - {test}")
                all_failures.append({'run': run_name, 'test': test, 'run_id': run_id})
            print()

    print(f"\n{'='*80}")
    print(f"TOTAL: {len(all_failures)} failed tests across {len(set(f['run'] for f in all_failures))} suites")
    print(f"{'='*80}")
...
if __name__ == '__main__':
    ...
    try:
        get_failures_by_run_ids(build_id, run_filters)
